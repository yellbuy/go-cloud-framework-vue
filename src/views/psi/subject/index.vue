						...
						<el-table-column prop="State" label="状态" width="80" align="center" show-overflow-tooltip>
							<template #default="scope">
								<el-switch
									...
									@change="proxy.$api.common.table.updateById('psi_subject', 'State', scope.row.Id, scope.row.State)"
									:active-text="$t('message.action.enable')"
									:inactive-text="$t('message.action.disable')"
									:active-value="1"
									:inactive-value="0"
						...
						<el-table-column prop="Order" label="排序" :width="90" align="center">
							<template #header>
								<el-button
									type="text"
									v-if="tableData.data"
									@click="proxy.$api.common.table.update('psi_subject', 'Order', tableData.data || [], 0)"
									v-auth:[moduleKey]="'btn.Edit'"
								>
									<el-icon>
										<Edit />
...
import childDlg from './component/subjectEdit.vue';
...
export default {
	...
	setup() {
		...
		const categoryKind=`subject_${kind}`;

		const scopeMode = route.params.scopeMode || 0;

		const scopeValue = route.params.scopeValue || 0;

		const moduleKey = `api_psi_subject`;
		...
		const onGetTableData = async (gotoFirstPage: boolean = false) => {
			...
			try {
				const res = await proxy.$api.psi.subject.getListByScope(state.kind, state.scopeMode, state.scopeValue, state.tableData.param);
				if (res.errcode != 0) {
					return;
				}
		...
		const onChildDel = (Id: string) => {
			ElMessageBox.confirm(`确定要删除当前记录吗?`, '提示', {
				...
				try {
					const res = await proxy.$api.psi.subject.delete(Id);
					if (res.errcode == 0) {
						onGetTableData();
					}
				} finally {

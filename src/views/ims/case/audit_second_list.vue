		...
		<el-card shadow="hover">
			...
			<el-table :data="tableData.data"  :span-method="objectSpanMethod"
				...
				<el-table-column :label="$t('message.action.operate')" :width="proxy.$calcWidth(160)" fixed="right">
					<template #default="scope">
						<el-button text bg type="info" v-if="scope.row.InsurerReviewState > 0" @click="onOpenEditDlg(false,scope.row)">
							<el-icon>
								<Search />
							</el-icon>
							&#8197;{{ $t('message.action.see') }}
						</el-button>
						<el-button text bg type="primary" v-if="scope.row.InsurerReviewState == 2" @click="onOpenEditDlg(true,scope.row)" v-auth:[moduleKey]="'btn.AuditEdit'">
							<el-icon>
								<Edit />
							</el-icon>

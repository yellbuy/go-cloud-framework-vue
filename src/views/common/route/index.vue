	...
	<div class="base-role-container">
		<el-card shadow="hover">
			...
			>
				...
				<el-table-column prop="OriginSiteName" label="发货地" show-overflow-tooltip fixed></el-table-column>
				<el-table-column prop="DestinationSiteName" label="收货地" show-overflow-tooltip></el-table-column>
				<el-table-column label="状态" width="70" align="center">
					<template #default="scope">
						<el-switch
							...
							@change="proxy.$api.common.table.updateById('common_route', 'state', scope.row.Id, scope.row.State)"
				...
				<el-table-column prop="Order" label="排序" :width="90" align="center">
					<template #header>
						<el-button
							type="text"
							v-if="tableData.data"
							@click="proxy.$api.common.table.update('common_route', 'Order', tableData.data || [], 0)"
							v-auth:[moduleKey]="'btn.Edit'"
						>
							<el-icon>
								<Edit />
				...
				<el-table-column prop="UpdateTime" label="更新时间" width="160" :formatter="dateFormatYMDHM" show-overflow-tooltip></el-table-column>
...
export default {
	...
	setup() {
		...
		const moduleKey = `api_common_route`;
		...
		const onGetTableData = async (gotoFirstPage: boolean = false) => {
			...
			try {
				const res = await proxy.$api.common.route.getListByScope(state.kind, state.scopeMode, state.scopeValue, state.tableData.param);
				if (res.errcode != 0) {
					return;
				}

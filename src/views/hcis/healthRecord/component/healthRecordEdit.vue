					...
					<el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb20">
						<el-form-item label="文化程度" prop="Education">
							<el-radio-group v-model="ruleForm.Education">
								<el-radio v-for="item in educationList" :key="item.Id" :label="item.Code">{{ item.Name
								}}</el-radio>
							</el-radio-group>
						</el-form-item>
					</el-col>
					<el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb20">
						<el-form-item label="居住情况（多选）" prop="Resident">
							<el-checkbox-group v-model="ruleForm.ResidentArray">
								<el-checkbox v-for="item in residentList" :key="item.Id" :label="item.Name">{{ item.Name
								}}</el-checkbox>
							</el-checkbox-group>
						</el-form-item>
					</el-col>
					<el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb20">
						<el-form-item label="婚姻状况" prop="Marriage">
							<el-radio-group v-model="ruleForm.Marriage">
								<el-radio v-for="item in marriageList" :key="item.Id" :label="item.Code">{{ item.Name
								}}</el-radio>
							</el-radio-group>
						</el-form-item>
					</el-col>
					<el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb20">
						<el-form-item label="医疗费用支付方式（多选）" prop="PayMode">
							<el-checkbox-group v-model="ruleForm.PayModeArray">
								<el-checkbox v-for="item in payModeList" :key="item.Id" :label="item.Name">{{ item.Name
								}}</el-checkbox>
							</el-checkbox-group>
						</el-form-item>
					</el-col>
					<el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb20">
						<el-form-item label="经济来源（多选）" prop="IncomeStream">
							<el-checkbox-group v-model="ruleForm.IncomeStreamArray">
								<el-checkbox v-for="item in incomeStreamList" :key="item.Id" :label="item.Name">{{ item.Name
								}}</el-checkbox>
							</el-checkbox-group>
						</el-form-item>
					</el-col>
					...
					<el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb20">
						<el-form-item label="不良生活习惯（多选）" prop="BadHabitArray">
							<el-checkbox-group v-model="ruleForm.BadHabitArray">
								<el-checkbox v-for="item in badHabitList" :key="item.Id" :label="item.Name">{{ item.Name
								}}</el-checkbox>
							</el-checkbox-group>
						</el-form-item>
					</el-col>
		...
		const onSubmit = (isCloseDlg: boolean) => {
			proxy.$refs.ruleFormRef.validate(async (valid: any) => {
				if (valid) {
					...
					state.ruleForm.PayMode = (state.ruleForm.PayModeArray && state.ruleForm.PayModeArray.length > 0) ? state.ruleForm.PayModeArray.join(",") : "";
					state.ruleForm.IncomeStream = state.ruleForm.IncomeStreamArray.join(",") || ""
					state.ruleForm.BadHabit = (state.ruleForm.BadHabitArray && state.ruleForm.BadHabitArray.length > 0) ? state.ruleForm.BadHabitArray.join(",") : "";
					if (state.Files) {

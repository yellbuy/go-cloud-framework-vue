		...
		<el-row :gutter="20" class="mt10 ml10">
			<el-col :span="10" class="mt10">
				<el-timeline>
					...
					<el-timeline-item placement="top" type="primary" icon="MoreFilled" :timestamp="tenderCertificate.AuditState > 0 ? tenderCertificate.AuditTime : ''">
						...
						<div v-else-if="ruleForm.BidFee != 0">
							<div v-if="tenderCertificate.State == 0 || tenderCertificate.AuditState == 2">
								<span>请上传标书费凭证 ({{ ruleForm.BidFee }}元)<el-tag class="ml-2" type="danger" v-if="tenderCertificate.AuditState == 2">
									未通过</el-tag>
								</span>
								<div style="width: 50%">
									<el-upload
							...
							<div v-else>
								<span>
									当前标书费状态
									<el-tag class="ml-2" type="info" v-if="tenderCertificate.AuditState == 0">待审核</el-tag>
									<el-tag class="ml-2" type="success" v-else-if="tenderCertificate.AuditState == 1">通过</el-tag>
								</span>
					...
					<el-timeline-item placement="top" v-if="projectCompany.Step == bond || projectCompany.Step == qualifications" type="primary" icon="MoreFilled" :timestamp="bondCertificate.AuditState > 0 ? bondCertificate.AuditTime : ''">
						...
						<div v-else-if="ruleForm.EnsureFee != 0">
							<div v-if="bondCertificate.State == 0 || bondCertificate.AuditState == 2">
								<span>
									请上传保证金凭证 ({{ ruleForm.EnsureFee }}万元)
									<el-tag class="ml-2" type="danger" v-if="bondCertificate.AuditState == 2">未通过</el-tag>
								</span>
								<div style="width: 50%">
									<el-upload
										class="avatar-uploader"
							...
							<div v-else>
								<span>当前招标保证金状态
									<el-tag class="ml-2" type="info" v-if="bondCertificate.AuditState == 0">待审核</el-tag>
									<el-tag class="ml-2" type="success" v-else-if="bondCertificate.AuditState == 1">通过</el-tag>
								</span>
								<div class="demo-image__preview">
					...
					<el-timeline-item placement="top" v-if="projectCompany.Step == qualifications">
						<el-form-item label="公司报价(万元)：" prop="QuotedPrice"
							...
							<el-button style="margin-left: 10px" v-if="isTimeShow(ruleForm.FinishTime, new Date())" type="primary" @click="onSubmit(false, quotation)">{{ $t('message.action.save') }}</el-button>
							<el-button type="primary" v-if="isTimeShow(ruleForm.FinishTime, new Date())" @click="onSubmit(true, quotation)">{{$t('message.action.submit')}}</el-button>
						</el-form-item>
						<p title="" class="color-info-light font10">
							<SvgIcon name="fa fa-info-circle" class="mr3" />
							采购控制价
							<span style="color: #ff0000">{{ ruleForm.PurchasePrice }}</span>
							万元,当前公司报价
							<span style="color: #19d319">{{ projectCompany.QuotedPrice }}</span>
							万元
						</p>
						...
						<el-table :data="tableData.data" v-loading="tableData.loading" style="width: 100%" :height="proxy.$calcMainHeight(-500)" border stripe highlight-current-row>
							...
							<el-table-column v-if="isTimeShow(ruleForm.FinishTime, new Date())" :label="$t('message.action.operate')" :width="proxy.$calcWidth(190)"fixed="right">
								<template #default="scope">

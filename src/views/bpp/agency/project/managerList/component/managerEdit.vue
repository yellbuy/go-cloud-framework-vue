<template>
	<div class="system-edit-user-container">
		<el-dialog :title="state.title" v-model="state.isShowDialog" destroy-on-close width="450px" :before-close="onCancel">
			<el-form ref="ruleFormRef" :model="state.ruleForm" :rules="rules" size="small" label-width="120px" v-loading="state.loading">
				<el-row :gutter="20">
					...
					<el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb12">
						<el-form-item label="所属部门" prop="SupplierIds">
							<el-select multiple v-model="state.SupplierIds" placeholder="请选择">
								<el-option v-for="item in state.sectionList" :key="item.Id" :label="item.Name" :value="item.Id" />
							</el-select>
						</el-form-item>
					</el-col>
					<el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24" class="mb12">
						<el-form-item label="手机号" prop="Mobile">
							<el-input v-model="state.ruleForm.Mobile"/>
						</el-form-item>
					</el-col>
				</el-row>
			</el-form>
...
const GetByIdRow = async (id: string) => {
	try {
		const res = await proxy.$api.base.user.getById(id);
		if (res.errcode != 0) {
			return;
		}

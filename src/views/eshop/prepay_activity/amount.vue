		...
		<el-card shadow="hover">
			...
			<el-table :data="tableData.data" v-loading="tableData.loading"
				...
				<el-table-column prop="State" label="有效" width="80" align="center">
					<template #default="scope">
						<el-switch v-model="scope.row.IsOnSale" inline-prompt v-auth:[moduleKey]="'btn.Edit'" :width="50"
						@change="proxy.$api.common.table.updateById('eshop_prepay_activity','State',scope.row.Id,scope.row.IsOnSale)" 
				...
				<el-table-column prop="Order" label="排序" width="100" align="center">
					<template #header>
						<el-button  type="text" v-if="tableData.data" 
							@click="proxy.$api.common.table.update('eshop_prepay_activity','Order', tableData.data||[], 0)" v-auth:[moduleKey]="'btn.Edit'">
							<el-icon>
								<Edit />
							</el-icon>
				...
				<el-table-column prop="StartTime" label="活动开始时间" width="120" :formatter="dateFormatYMDHM" show-overflow-tooltip>
				</el-table-column>
				<el-table-column prop="EndTime" label="活动结束时间" width="120" :formatter="dateFormatYMDHM" show-overflow-tooltip>
				</el-table-column>
...
import dlgEdit from './component/amountEdit.vue';
...
export default {
	...
	setup() {
		...
		const onOpenEditDlg = async (row: Object) => {
			if(!row){
				row = { Kind: state.kind,state:1, Amount:0, Point:0, PayAmount:0 }
			}

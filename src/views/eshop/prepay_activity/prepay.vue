		...
		<el-card shadow="hover">
			...
			<el-table :data="tableData.data" v-loading="tableData.loading"
				...
				<el-table-column prop="State" label="有效" width="80" align="center">
					<template #default="scope">
						<el-switch v-model="scope.row.State" inline-prompt v-auth:[moduleKey]="'btn.Edit'" :width="50"
...
export default {
	...
	setup() {
		...
		const onOpenEditDlg = async (row: Object) => {
			if(!row){
				row = { Kind: state.kind,state:1, ActMode:actMode, ActType: actType, Amount:0, Point:0, PayAmount:0 }
			}

...
export default {
	 ...
	 */
	 getListByScope:async (kind:number,scopeMode:number=0,scopeValue:number=0,params: object={})=>{
		const url=`/v1/admin/eshop/ad/${kind}/${scopeMode}/${scopeValue}`;
    ...
    getById:async (id:number|string)=>{
		const url=`/v1/admin/eshop/ad/${id}`;
	...
	update:async(data:object)=>{
		const url=`/v1/admin/eshop/ad/${data.Id}`;
		return await http.post(url, data);
	},
	/**
	 * 新增或更新
	...
	delete:async(ids:number|string|[])=>{
		if(ids instanceof Array){
			const url=`/v1/admin/eshop/ad/delete`;
			return await http.post(url,ids);
		}
		const url=`/v1/admin/eshop/ad/delete/${ids}`;

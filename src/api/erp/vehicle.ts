...
export default {
    ...
    /**
         ...
         * @returns 返回接口数据
         */
    getStateStat: async (kind: string, scopeMode: number = 0, scopeValue: number = 0, params: object = {}) => {
        const url = `/v1/admin/erp/vehicle/stat/state/${kind}/${scopeMode}/${scopeValue}`;
        return await http.get(url, params);
    },
     ...
     * @returns 返回接口数据
     */
    getVehicleWarningList: async (kind: string, scopeMode: number = 0, scopeValue: number = 0, params: object = {}) => {
        const url = `/v1/admin/erp/vehicle/warning/list/${kind}/${scopeMode}/${scopeValue}`;
        return await http.get(url, params);
    },

...
export default {
    ...
    getListByScope: async (kind: string, scopeMode: number = 0, scopeValue: number = 0, params: object = {}) => {
        const url = `/v1/admin/erp/energybill/${kind}/${scopeMode}/${scopeValue}`;
    ...
    getById: async (id: number | string,loadRelated:boolean=false) => {
		const url = `/v1/admin/erp/energybill/${id}?loadRelated=${loadRelated}`;
    ...
    save: async (data: object) => {
        if (!data.Id || data.Id == "0") {
            const url = `/v1/admin/erp/energybill`;
            return await http.post(url, data);
        }
        const url = `/v1/admin/erp/energybill/${data.Id}`;
        return await http.post(url, data);
    },
    /**
         * 新增或更新
    ...
    copy: async (id:string|number) => {
        ...
        const url = `/v1/admin/erp/energybill/copy/${id}`;
    ...
    delete: async (id: number | string) => {
        const url = `/v1/admin/erp/energybill/delete/${id}`;
        return await http.post(url);
    },
}
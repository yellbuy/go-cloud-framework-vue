...
export default {
    ...
    getListByScope: async (kind: string, scopeMode: number = 0, scopeValue: number = 0,params: object = {}) => {
        const url = `/v1/admin/erp/projecttask/${kind}/${scopeMode}/${scopeValue}`;
    ...
    getById: async (id: number | string) => {
        const url = `/v1/admin/erp/projecttask/${id}`;
    ...
    save: async (data: object) => {
        if (!data.Id || data.Id == "0") {
            const url = `/v1/admin/erp/projecttask`;
    ...
    delete: async (id: number | string) => {
        const url = `/v1/admin/erp/projecttask/delete/${id}`;

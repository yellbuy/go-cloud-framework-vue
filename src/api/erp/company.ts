...
export default {
    ...
    getListByScope: async (kind: string, scopeMode: number = 0, scopeValue: number = 0, params: object = {}) => {
        const url = `/v1/admin/erp/company/${kind}/${scopeMode}/${scopeValue}`;
    ...
    save: async (data: object) => {
        if (!data.Id || data.Id == "0") {
            const url = `/v1/admin/erp/company`;
    ...
    delete: async (id: number | string) => {
        const url = `/v1/admin/erp/company/delete/${id}`;
   ...
   */
    disable: async (data: object) => {
        const url = `/v1/admin/erp/company/disable`;
        return await http.post(url, data);
    },
}
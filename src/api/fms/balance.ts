...
export default {
    ...
    getListByScope: async (kind: string, scopeMode: number = 0, scopeValue: number = 0, params: object = {}) => {
        const url = `/v1/admin/fms/balance/${kind}/${scopeMode}/${scopeValue}`;
     ...
     * @returns 返回接口数据
     */
    getById: async (kind:string,id: number | string) => {
        const url = `/v1/admin/fms/balance/${kind}/${id}`;
    ...
    save: async (data: object) => {
        const url = `/v1/admin/fms/balance/save/${data.Id}`;
    ...
    delete: async (id: number | string) => {
        const url = `/v1/admin/fms/balance/delete/${id}`;

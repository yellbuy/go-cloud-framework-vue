...
/**
 * 设置后端默认情况下有权限的路由到 vuex routesList 中（已处理成多级嵌套路由）及缓存多级嵌套数组处理后的一维数组
 * @description 用于左侧菜单、横向菜单的显示
 * @description 用于 tagsView、菜单搜索中：未过滤隐藏的(isHide)
 */
 export function setFilterMenuAndCacheTagsViewRoutesForBackEnd() {
	store.dispatch('routesList/setRoutesList', setFilterMenu(dynamicRoutes[0].children));
...
export function setFilterRoute(child: any) {
	let filterRoute: any = [];
	console.log("setFilterRoute child:",child)
	if(child){
...
router.beforeEach(async (to, from, next) => {
	...
	if ((to.path === '/login' || to.path === '/register' || to.path === '/changepassword') && !token) {
		//获取APPID
